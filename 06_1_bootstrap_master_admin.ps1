...
# DR. CHASI - BOOTSTRAP MASTER ADMIN
#
# Creates the first Supabase Auth user for the master administrator and
# links it to public.staff_profiles.
#
# SECURITY:
# - No password is stored in this file.
# - No service_role key is stored in this file.
# - Both secrets are requested interactively.
# - Do NOT commit service_role credentials to GitHub.

$ErrorActionPreference = "Stop"

Write-Host ""
Write-Host "==============================================" -ForegroundColor Cyan
Write-Host " DR. CHASI - CREATE MASTER ADMIN" -ForegroundColor Cyan
Write-Host "==============================================" -ForegroundColor Cyan
Write-Host ""
...
$ServiceRoleSecure = Read-Host "Supabase service_role key" -AsSecureString
$ServiceRoleKey = Convert-SecureToPlainText $ServiceRoleSecure

if ([string]::IsNullOrWhiteSpace($ServiceRoleKey)) {
    throw "service_role key is required."
}
...
Write-Host "master"
Write-Host ""
Write-Host "The password and service_role key were NOT written to disk." -ForegroundColor Green
Write-Host ""
Write-Host "Next step: connect /admin/login to Supabase Auth." -ForegroundColor Cyan

# Best-effort cleanup of plaintext secrets in the current PowerShell process.
$MasterPassword = $null
$ServiceRoleKey = $null
